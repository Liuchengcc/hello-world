...
Controller::Controller(QObject *parent) : QObject(parent),
    ...
    sysState_(sys_unkown),
    servoState_(servo_unkown),
    modeState_(mode_unkown),
    coordMode_("j"),
    jogMode_(Jog),
    teachSpeed_(30)
{
    ...
    connect(tcpModule_, &TcpLink::tcpConnected, this, []
    {
...
{
    auto getValueFromRecvData = [&recvData](int index)
    {
        QStringList recvDataList = recvData.split(RECVDATA_SPLIT1);
...
#define SET_OBJ_VALUE(VALUE, CURRENT_VALUE, SET_FUNC) if(VALUE != CURRENT_VALUE)SET_FUNC(VALUE);
    SET_OBJ_VALUE((SysState)getValueFromRecvData(0).toInt(), get_sysState_(), set_sysState_);
    SET_OBJ_VALUE((ModeState)getValueFromRecvData(1).toInt(), get_modeState_(), set_modeState_);
    SET_OBJ_VALUE((ServoState)getValueFromRecvData(6).toInt(), get_servoState_(), set_servoState_);
    SET_OBJ_VALUE(getValueFromRecvData(4), get_ethercatError_(), set_ethercatError_);
    SET_OBJ_VALUE(getValueFromRecvData(5), get_servoError_(), set_servoError_);
    SET_OBJ_VALUE(getValueFromRecvData(7), get_motionError_(), set_motionError_);
    SET_OBJ_VALUE(getValueFromRecvData(8), get_interpreterError_(), set_interpreterError_);
}
